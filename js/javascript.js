...
// LOGOのサイズをブラウザのサイズによって変える
    var hsize = parseInt($(window).height());
    var wsize = parseInt($(window).width());
    if(hsize <= wsize){
        $("img.logo").css('height', hsize - 40 + 'px');
        $("img.logo").css('width', '');
    }else{
        $("img.logo").css('width', wsize + 'px');
        $("img.logo").css('height', '');
    }

});
// ブラウザのサイズが変更されたらLOGOのサイズをブラウザのサイズによって変える
$(window).resize(function () {
    var hsize = parseInt($(window).height());
    var wsize = parseInt($(window).width());
    if(hsize <= wsize){
